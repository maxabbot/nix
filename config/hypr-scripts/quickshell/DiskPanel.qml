...
Item {
    ...
    property var smart: []   // [{name, model, passed, temp, wear}] from /run/smart
    ...
    function refresh() { lsblkProc.running = true; smartProc.running = true }
    Timer { id: refreshTimer; interval: 5000; repeat: true; onTriggered: root.refresh() }

    // Fixed-disk SMART health, written by the root `smart-status` timer (see
    // productivity.nix) — this panel only reads the reduced summary, no root.
    Process {
        id: smartProc
        command: ["bash", "-c", "cat /run/smart/summary.json 2>/dev/null || echo '[]'"]
        stdout: StdioCollector {
            onStreamFinished: {
                try { root.smart = JSON.parse(text) || [] }
                catch (e) { /* transient write — keep last list */ }
            }
        }
    }
        ...
        Column {
            ...
            Repeater {
                ...
                delegate: Rectangle {
                    ...
                    height: 44
                    radius: 8
                    color: modelData.passed === false ? Theme.redDark : Theme.bgAlt
                    ...
                    RowLayout {
                        ...
                        Text {
                            text: "󰋊"
                            color: modelData.passed === false ? Theme.red : Theme.gray
                            font.pixelSize: 18
                            font.family: Theme.font
                        }
                        ...
                        ColumnLayout {
                            ...
                            Text {
                                text: {
                                    var s = []
                                    if (modelData.temp !== null && modelData.temp !== undefined)
                                        s.push(modelData.temp + "°C")
                                    if (modelData.wear !== null && modelData.wear !== undefined)
                                        s.push(modelData.wear + "% worn")
                                    return s.join("  ·  ")
                                }
                                visible: text !== ""
                                color: Theme.grayDim
                        ...
                        Text {
                            text: modelData.passed === true ? "Healthy"
                                : modelData.passed === false ? "FAILING" : "—"
                            color: modelData.passed === true ? Theme.green
                                : modelData.passed === false ? Theme.red : Theme.gray
                            font.pixelSize: 11
                            font.bold: modelData.passed === false
                            font.family: Theme.font
                        }
                    }
                }
            ...
            Item { height: 4 }  // gap before the removable section
        }
