...
Item {
    ...
    property bool   hotspotAvail:  false // a Wi-Fi device exists to share from
    ...
    Process {
        id: pollHotspot
        // Hidden unless a Wi-Fi device exists (desktops on ethernet skip it). The
        // AP connection created by `nmcli device wifi hotspot` is named "Hotspot".
        command: ["bash", "-c",
            "nmcli -t -f TYPE dev status 2>/dev/null | grep -q '^wifi$' || { echo noavail; exit 0; }; "
            + "nmcli -t -f NAME,TYPE con show --active 2>/dev/null | grep -q '^Hotspot:' && echo up || echo down"]
        stdout: SplitParser {
            onRead: (line) => {
                var s = line.trim()
                if (s === "noavail") { root.hotspotAvail = false; return }
                root.hotspotAvail = true
                if (s === "up" || s === "down") root.hotspotOn = (s === "up")
            }
        }
    }
    ...
    function runHotspot(on) {
        // `nmcli device wifi hotspot` auto-generates SSID/password and activates
        // the "Hotspot" connection (this drops the Wi-Fi station link on a single
        // radio — expected). Off tears the connection back down.
        setHotspot.command = ["bash", "-c", on
            ? "nmcli device wifi hotspot 2>/dev/null || true"
            : "nmcli connection down Hotspot 2>/dev/null || true"]
        setHotspot.running = true
        root.hotspotOn = on
    }
    ...
    readonly property var tiles: {
        ...
        if (root.hotspotAvail)   t.push({ action: "hotspot" })
        return t
    }
                ...
                delegate: Rectangle {
                    ...
                    readonly property bool tileActive: {
                        switch (modelData.action) {
                            ...
                            case "hotspot":   return root.hotspotOn
                    ...
                    readonly property string tileLabel: {
                        switch (modelData.action) {
                            ...
                            case "hotspot":   return root.hotspotOn ? "Hotspot On" : "Hotspot"
                            default:          return ""
                        }
                    }
                        ...
                        onClicked: {
                            switch (modelData.action) {
                                ...
                                case "tailscale":
                                    ...
                                    break
                                case "hotspot":
                                    root.runHotspot(!root.hotspotOn)
                                    break
                            }
                        }
                    }
