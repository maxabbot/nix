...
Item {
    ...
    property string tzName:  ""   // IANA zone from timedatectl
    property bool ntpEnabled: false
    property bool ntpSync:   false
...
if command -v timedatectl >/dev/null 2>&1; then
  # --value with three -p keys prints Timezone, NTP, NTPSynchronized on
  # separate lines; join to one "time <tz> <ntp> <sync>" record.
  timedatectl show -p Timezone -p NTP -p NTPSynchronized --value 2>/dev/null |
    tr '\\n' ' ' | awk 'NF>=3{print "time", $1, $2, $3}'
fi
`]
        stdout: SplitParser {
            onRead: (line) => {
                ...
                switch (p[0]) {
                    ...
                    case "time":    root.tzName = p[1]; root.ntpEnabled = (p[2] === "yes"); root.ntpSync = (p[3] === "yes"); break
                }
            }
        }
    }
    ...
    Column {
        ...
        Column {
            ...
            RowLayout {
                ...
                Text {
                    // Green only when NTP is enabled *and* the clock has synced;
                    // amber if enabled but not yet synced; gray when disabled.
                    text: !root.ntpEnabled ? "disabled"
                        : root.ntpSync ? "synced" : "syncing…"
                    color: !root.ntpEnabled ? Theme.gray
                        : root.ntpSync ? Theme.green : Theme.yellow
                    font.pixelSize: 11
                    font.family: Theme.font
                }
            }
