...
{ pkgs, lib, machineType, compositor, monitors, git, location, inputs, ... }:
{
  imports = [
  ...
  home = {
    ...
    packages = with pkgs; [
      ...
      nix-tree nix-diff nixpkgs-review
    ];
  };
  ...
  programs.ripgrep = {
    enable   = true;
    arguments = [ "--smart-case" "--follow" "--hidden" "--glob=!.git" ];
  };
