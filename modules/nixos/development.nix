...
in {
  ...
  config = lib.mkIf cfg.enable {
    environment.systemPackages = with pkgs; [
      ...
      # Dev utilities
      kitty direnv helix shellcheck tig imagemagick sqlite
    ...
    ++ lib.optionals cfg.containers.libvirt.enable [
      libvirt qemu virt-manager dnsmasq
    ]
    ++ lib.optionals cfg.cloudTools.enable [
      kubectl kubectx kubernetes-helm opentofu ansible
      awscli2 azure-cli google-cloud-sdk doctl
    ]
    ++ lib.optionals cfg.database.servers.enable [
      mongodb
    ]
    ++ lib.optionals cfg.database.guiClients.enable [
      dbeaver-bin beekeeper-studio mycli litecli
    ]
