...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    # ── XDG portals ────────────────────────────────────────────────────────────
    xdg.portal.extraPortals = lib.optionals (cfg.compositor == "hyprland") [
    ...
    environment.systemPackages = with pkgs; [
      # Wayland tools
      grim slurp swww swayidle swaynotificationcenter
      wl-clipboard wl-paste cliphist fuzzel gammastep
      wlogout nwg-look hyprlock waybar playerctl
      brightnessctl grimblast

      # Audio
      pavucontrol

      # File manager
      xfce.thunar xfce.thunar-archive-plugin file-roller

      # Office / documents
      libreoffice-fresh xournalpp zathura zathura-pdf-mupdf
      calibre pdfarranger masterpdfeditor onlyoffice-bin
      ...
      # Notes / passwords
      obsidian bitwarden-desktop

      # Media
      vlc mpv imv mpvpaper

      # Cloud / sync
      rclone

      # Monitoring / hardware
      nvtop openrgb-with-all-plugins glances

      # Security / backup
      veracrypt

      # Desktop helpers
      kvantum papirus-icon-theme foot
      syncthingtray
    ...
    ++ lib.optionals cfg.streamingTools.enable [
      obs-studio shotcut rustdesk gpu-screen-recorder losslesscut-bin
    ]
