...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.power-profiles-daemon.enable =
      cfg.powerManagement == "power-profiles-daemon";
    ...
    environment.systemPackages = with pkgs; [
      # Core utilities
      coreutils findutils gnused gnutar bc curl wget
      rsync openssh gnupg man-db man-pages bash-completion

      # Networking tools
      inetutils dnsutils traceroute nethogs iotop
      wireguard-tools openvpn networkmanagerapplet

      # Filesystem
      dosfstools exfatprogs btrfs-progs p7zip unzip unrar zip gparted parted

      # Wayland / display
      xdg-utils xdg-user-dirs wl-clipboard
      qt5.qtwayland qt6.qtwayland
      libsForQt5.qt5ct kdePackages.qt6ct

      # CLI essentials
      git git-lfs bat eza fd fzf ripgrep jq nano
      zsh tmux fastfetch btop delta libnotify
